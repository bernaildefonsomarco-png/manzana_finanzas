...
| `C-03` | 14 tools vs. 15 tools — documentación de capacidades desactualizada. | **Cerrada, cambiando el método y no el número.** El catálogo se genera por agregación desde las §14 de los dieciséis módulos y **un test falla el build** si se desincroniza (`40` §2, `AC-CATALOGO-01`). Contar bien una vez no habría servido: el defecto era mantener dos listas a mano. La cifra real, verificada mecánicamente en `W-16` (`WEB-D254`), son 99 comandos y 156 entradas de lectura; la antigua contaba "tools" de otra arquitectura, y el "95/145" de la primera redacción de `40` seguía siendo un conteo manual de filas de tabla, no de comandos distintos. Al agregar aparecieron además 7 colisiones de nombre, 1 de comando, 5 medidas con dos dueños y 3 módulos sin declarar sus prohibiciones — todo corregido en la Ola 10 (`40` §9). | `40_catalogo_de_tools_y_comandos.md` | `TEST` (`AC-CATALOGO-01` a `AC-CATALOGO-03`) |
...
| `C-09` | Lifecycle V1 documentado vs. solo onboarding inicial + drafts — retención no implementada como sistema. | **Cerrada declarando el alcance real, que es lo que faltaba.** En V1-web todo lo que sale del producto son **cuatro transaccionales y diez tipos de notificación apagados por defecto**, y nada más: sin secuencia de bienvenida, sin serie de activación, sin recordatorios de configuración incompleta (`RUL-ONB-05`), sin reactivación más allá del tipo `sin_registrar`, que viene apagado. El sistema D1-D30 de `15_retencion_lifecycle.md` se difiere a la fase 2 **por decisión, no por omisión**: un ciclo de vida por correo sin el canal principal es una campaña, y las campañas están prohibidas (`RUL-MAIL-09`). Declarar que algo no se hará y por qué es lo que la cierra; dejarla como pendiente indefinido es lo que la mantenía abierta. | `46_notificaciones_y_correo_saliente.md` (`RUL-MAIL-10`), `44_onboarding_web.md` | `DOC` + `TEST` (`AC-MAIL-15`) |
...
| `C-17` | Proactivos "activados por defecto" (tabla antigua de `05a`) vs. consentimiento atómico y gate live posteriores — se resuelve a favor de la regla posterior. | **Cerrada.** `RUL-NOTIF-04` fija que ningún canal que interrumpa viene activado, y el consentimiento del correo es **por tipo, no global** (`WEB-D070`): diez interruptores, los diez apagados al crear la cuenta, visibles vacíos en `SCR-NOTIF-03`. Se verifica con `AC-NOTIF-01` y `AC-NOTIF-02`. Y `WEB-D073` lo completa por el otro lado: la preferencia se lee **al enviar**, no al encolar, para que apagar el interruptor tenga efecto inmediato. La tabla antigua de `05a` que los daba por activados no se hereda. | `37_modulo_recordatorios_in_app.md`, `46_notificaciones_y_correo_saliente.md` | `TEST` + `USER` |
