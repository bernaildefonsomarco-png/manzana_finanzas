...
// Criterio agregado (`51` §5): su conjunto son las 60 rutas de `/api/v1`
// (`AC-SEG-04` no se cierra con una prueba, se cierra con la unión de que
// ninguna de las 60 use 403 para "no es tuyo" — que es exactamente lo que
...
describe("AC-SEG-04 (agregado): ninguna de las 60 rutas de /api/v1 usa 403 para un recurso ajeno", () => {
  const rutas = recorrer(RAIZ_V1, []);

  it("el conjunto declarado tiene 60 rutas — si cambia, hay que revisar esta prueba", () => {
    expect(rutas.length).toBe(60);
  });
