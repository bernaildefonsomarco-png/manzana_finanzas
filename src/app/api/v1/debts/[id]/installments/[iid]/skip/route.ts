...
import { SkipDebtInstallmentRequestSchema } from "../../../schemas";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!idempotencyKey) {
      return errorJson(
        "VALIDATION_ERROR",
        "Falta Idempotency-Key para saltar la cuota.",
        meta,
        400
      );
    }
    const params = ParamsSchema.parse(await context.params);
    const parsed = SkipDebtInstallmentRequestSchema.parse(
    ...
    const result = await skipDebtInstallment(createServiceClient(), {
      userId: auth.userId,
      installment,
