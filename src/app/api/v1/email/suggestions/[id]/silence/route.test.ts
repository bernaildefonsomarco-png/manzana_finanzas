...
function request() {
  return new Request(`http://localhost/api/v1/email/suggestions/${SUGGESTION_ID}/silence`, { method: "POST" });
}
function context() {
...
describe("POST /api/v1/email/suggestions/[id]/silence", () => {
  it("camino feliz: silencia la sugerencia", async () => {
    mocks.resolveSenderSuggestion.mockResolvedValue({ id: SUGGESTION_ID, status: "silenced" });
  ...
  it("idempotencia: silenciar dos veces la segunda devuelve 404, no un doble efecto", async () => {
    mocks.resolveSenderSuggestion
      .mockResolvedValueOnce({ id: SUGGESTION_ID, status: "silenced" })
