...
const BodySchema = z.object({ value: z.enum(["util", "no_util"]) }).strict();
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!idempotencyKey) {
      return errorJson("VALIDATION_ERROR", "Falta Idempotency-Key para guardar tu respuesta.", meta, 400);
    }
    ...
    if (error instanceof InsightOperationError) {
      return errorJson("CONFLICT", "Esa Idempotency-Key ya se uso con otros datos.", meta, 409);
    }
