...
const BodySchema = z.object({ reason: z.string().trim().max(240).optional() }).strict();
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!idempotencyKey) return errorJson("VALIDATION_ERROR", "Falta Idempotency-Key para descartar.", meta, 400);
