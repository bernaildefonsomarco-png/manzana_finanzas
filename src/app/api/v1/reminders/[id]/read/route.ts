...
import { markReminderRead, ReminderRepositoryError } from "@/data/repositories/reminders.repository";
...
// POST /reminders/[id]/read (37 §10): idempotente.
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    await markReminderRead(auth.client, auth.userId, id);
    return okJson({ id, read: true }, meta);
  } catch (error) {
