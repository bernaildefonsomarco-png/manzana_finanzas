...
/**
 * Nucleo compartido de "reescribir con `ResponseAgent`, con guardrails" (21
 * S6): construye el paquete de contexto, corre el reintento acotado y
 * valida que el texto candidato no invente ni pierda datos criticos frente
 * al texto base. Ningun canal le pasa la forma de su mensaje (botones,
 * bloques) — solo el texto a mejorar — asi que este archivo no conoce
 * ninguna forma de canal concreta.
 */

export type ResponseAgentEnhancementOutcome =
  | {
...
// Deriva del canal ya tipado en el paquete de contexto del agente
// (`ResponseContextPack["channel"]`, fuera de `src/core/`) en vez de
// repetir aqui la lista de canales concretos.
export type ResponseAgentEnhancementChannel = ResponseContextPack["channel"];

/**
 * Corre el `ResponseAgent` sobre `baseText` y devuelve el texto mejorado
 * (o `baseText` sin cambios si el intento se rechaza o falla) junto con la
 * traza de lo que paso. Nunca lanza: los errores del proveedor se
 * capturan y se reportan como `status: "failed"`.
 */
export async function enhanceResponseText(params: {
  baseText: string;
  channel: ResponseAgentEnhancementChannel;
  reason: string;
...
}): Promise<{ text: string; trace: ResponseAgentEnhancementTrace }> {
...
export function notApplicableEnhancementTrace(
  reason: "not_sendable" | "disabled",
): ResponseAgentEnhancementTrace {
  return {
    status: "not_applicable",
